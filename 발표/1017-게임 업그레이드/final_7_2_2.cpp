...
int getPlayerInput(int playerUsed[], int playerCount);
int getComputerInput(int computerUsed[], int computerCount, int lastPlayerInput, int difficulty);
...
void gameLoop(int difficulty) {
    int total = 0;
    int playerUsed[MAX_INPUT];      // 플레이어가 사용한 숫자
    int computerUsed[MAX_INPUT];    // 컴퓨터가 사용한 숫자
    int playerCount = 0;
    int computerCount = 0;
    int input;
    int isPlayerTurn = 1;
    ...
    while (total < TARGET_SCORE) {
        ...
        if (computerCount == 0) {
            ...
            for (int i = 0; i < computerCount; i++) {
                printf("%d ", computerUsed[i]);
            }
        }
        printf("\n");

        if (isPlayerTurn) {
            input = getPlayerInput(playerUsed, playerCount);
            playerUsed[playerCount++] = input;
            lastPlayerInput = input;
        }
        else {
            input = getComputerInput(computerUsed, computerCount, lastPlayerInput, difficulty);
            computerUsed[computerCount++] = input;
        }
